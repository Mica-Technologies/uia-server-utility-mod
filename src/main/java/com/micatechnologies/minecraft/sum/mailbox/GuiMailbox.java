package com.micatechnologies.minecraft.sum.mailbox;

import net.minecraft.client.gui.inventory.GuiContainer;
import net.minecraft.client.renderer.GlStateManager;

/**
 * Mailbox GUI. Title varies by viewer ("Your Mailbox" for owner, "Mailbox: <owner>" for
 * everyone else). Slot frames render the same in both modes; the deposit-only restriction
 * is enforced by {@link ContainerMailbox} via Slot subclassing, not by the GUI.
 */
public class GuiMailbox extends GuiContainer {

    private static final int GUI_W = 176;
    private static final int GUI_H = ContainerMailbox.GUI_HEIGHT;

    private static final int BG_OUTER = 0xFF1A1D22;
    private static final int BG_INNER = 0xFF40444C;
    private static final int TEXT_TITLE = 0xFFE0E0E0;
    private static final int TEXT_DIM = 0xFF9098A0;
    private static final int TEXT_HINT = 0xFFB0B870;

    private final TileEntityMailbox mailbox;
    private final boolean ownerView;

    public GuiMailbox(ContainerMailbox container) {
        super(container);
        this.mailbox = container.getMailbox();
        this.ownerView = container.isOwnerView();
        this.xSize = GUI_W;
        this.ySize = GUI_H;
    }

    @Override
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        drawRect(guiLeft, guiTop, guiLeft + GUI_W, guiTop + GUI_H, BG_OUTER);
        drawRect(guiLeft + 3, guiTop + 3, guiLeft + GUI_W - 3, guiTop + GUI_H - 3, BG_INNER);

        // Inbox row of 9 slots
        for (int col = 0; col < TileEntityMailbox.INBOX_SIZE; col++) {
            drawSlotFrame(guiLeft + 7 + col * 18, guiTop + 17);
        }
        // Player inv 3 rows
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                drawSlotFrame(guiLeft + 7 + col * 18,
                    guiTop + ContainerMailbox.PLAYER_INV_Y - 1 + row * 18);
            }
        }
        for (int col = 0; col < 9; col++) {
            drawSlotFrame(guiLeft + 7 + col * 18,
                guiTop + ContainerMailbox.PLAYER_HOTBAR_Y - 1);
        }
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
    }

    private void drawSlotFrame(int x, int y) {
        drawRect(x, y, x + 18, y + 18, 0xFF202326);
        drawRect(x + 1, y + 1, x + 17, y + 17, 0xFF303338);
    }

    @Override
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        if (ownerView) {
            drawString(this.fontRenderer, "Your Mailbox", 8, 6, TEXT_TITLE);
        } else {
            String name = mailbox.getOwnerName();
            drawString(this.fontRenderer,
                "Mailbox: " + (name == null || name.isEmpty() ? "?" : name),
                8, 6, TEXT_TITLE);
            drawString(this.fontRenderer,
                "Drop items in to send.", 8, 38, TEXT_HINT);
        }
        drawString(this.fontRenderer, "Inventory", 8,
            ContainerMailbox.PLAYER_INV_Y - 12, TEXT_DIM);
    }

    @Override
    public boolean doesGuiPauseGame() {
        return false;
    }
}
