...
%
%  Build:  pdflatex ECONOMIC_API.tex   (run twice for the table of contents)
%          or: latexmk -pdf ECONOMIC_API.tex
%
%  Uses only widely-available LaTeX2e packages; no manual installs required.
%  The authoritative source of this content is docs/ECONOMIC_API.md --- keep
...
SUM's client is configured under the \code{economy\_api} category in its Forge config file. All
keys are server-side; none are synced to clients. Changing any of them requires a server restart.

\begin{longtable}{L{4.4cm} L{1.5cm} L{1.6cm} L{6.4cm}}
\toprule
...
\midrule
\endhead
\code{enabled} & boolean & \code{false} &
  Master switch. When \code{false}, no remote connection is made and the local backends are used.\\
\code{allowIntegratedServer} & boolean & \code{false} &
  Whether a single-player or LAN world may contact the service. Off by default so a local save
  cannot spend from the shared economy. Minecraft clients never contact it regardless.\\
\code{sendIntegrityHeaders} & boolean & \code{true} &
  Send the optional anomaly-detection headers of Section~\ref{sec:integrity}.
  \code{X-MCE-Environment} is always sent and is not covered by this switch.\\
\code{baseUrl} & string & \code{""} &
  Service root, without the \code{/api/economic/v1} suffix. \textbf{Must be \code{https://}} unless \code{enableHttp} is set.\\
\code{authToken} & string & \code{""} &
  Bearer token. \textbf{A secret} --- protect this file accordingly.\\
\code{instanceId} & string & \code{"default"} &
  Identifies this Minecraft server; sent as \code{X-MCE-Instance}. Give each server a distinct value.\\
\code{hmacSecret} & string & \code{""} &
  Enables HMAC request signing when non-empty. \textbf{A secret.}\\
\code{certificatePath} & string & \code{""} &
  Certificate or CA bundle to trust in addition to the system trust store, for self-signed or
  private-CA services. \textbf{Relative to the config file}; absolute paths also accepted. PEM,
  PKCS\#12, or JKS.\\
\code{certificatePassword} & string & \code{""} &
  Password for \code{certificatePath} when it is a PKCS\#12 or JKS keystore. Not needed for PEM.
  \textbf{A secret.}\\
\code{enableHttp} & boolean & \code{false} &
  Permits a plaintext \code{http://} base URL. \textbf{Local development only} --- the token and
  every balance travel in the clear. Logs a warning on every startup while enabled.\\
\code{connectTimeoutMs} & integer & \code{3000} & TCP and TLS connect timeout.\\
\code{readTimeoutMs} & integer & \code{5000} & Response read timeout.\\
\code{maxRetries} & integer & \code{2} &
  Retries per operation for retryable failures, using the same idempotency key.\\
\code{balanceCacheTtlSeconds} & integer & \code{30} & Age at which a cached balance is refreshed.\\
\code{eventPollSeconds} & integer & \code{10} & \code{/getEvents} poll interval; \code{0} disables.\\
\code{healthPollSeconds} & integer & \code{60} & \code{/health} interval.\\
\code{unavailablePolicy} & string & \code{"deny"} & \code{deny}, \code{cached}, or \code{local}.\\
\code{verboseLogging} & boolean & \code{false} &
  Logs every request and response. Never logs the token or signature.\\
\bottomrule
\end{longtable}
