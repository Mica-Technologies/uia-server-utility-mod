...
| 14 | Bundle OneConfig wrapper into SUM jar (embed + manifest TweakClass entry) | ✅ landed | `eaabd0f` |
| 15 | Custom-text HUD placeholders + slot bump 5→10 (alternative to porting EvergreenHUD's full dynamic HudList framework) | ✅ landed | (this commit) |
| 16 | Server-config bridge (read-only OneConfig mirror of server settings) | ✅ landed | (this commit) |
...
- [x] **Task 15 — Custom-text HUD placeholders + slot bump.** ✅ Shipped (this session). Took the pragmatic alternative to porting EvergreenHUD's full dynamic `HudList` framework: instead of a custom OneConfig option type for dynamic-sized lists, bumped the fixed slot count from 5 to 10 AND added `%placeholder%` substitution to `CustomTextHud` via a new `TextPlaceholders` helper. Supported tokens: `%player%`, `%x%/%y%/%z%`, `%dim%`, `%biome%`, `%server%`, `%time%` (in-game 24h), `%realtime%` (wall-clock), `%date%`, `%fps%`, `%direction%`. Result: a single slot can render arbitrarily many live values (e.g. `"%player% @ %x%,%z% in %biome%"`), which addresses the "I need more custom text" use case far more practically than dynamic slot count would. Unknown tokens are left intact (`%foo%`) so users see typos immediately.

- [x] **Task 16 — Read-only OneConfig mirror of server-side settings.** ✅ Shipped (this session). New package `serverconfig/` with: `ServerConfigSnapshot` (flat data class + ByteBuf serialize, versioned for forward-compat), `PacketSyncServerConfig` (S→C, slot 17), `ServerConfigBridge` (server-side @SubscribeEvent on `PlayerLoggedInEvent` builds + pushes snapshot from `SumConfig`), `ServerConfigMirror` (client-side static cache), `GuiServerConfigViewer` (scrollable read-only display, sections mirror SumConfig categories). SumOneConfig surfaces a `Server Config / Mirror / Open Server Config Viewer` button — opening it pops the GuiScreen showing the latest snapshot. Why a Button + custom GuiScreen instead of `@Info` fields: OneConfig's `@Info.text()` is a compile-time annotation literal, so it can't display dynamic runtime values. The custom GuiScreen sidesteps that. Snapshot is one-shot per login (re-login after `/sum reloadconfig` to refresh); a push-on-reload path could be added later if useful.
