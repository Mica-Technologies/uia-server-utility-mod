...
from \code{UNKNOWN\_ACCOUNT}, which is reserved for identities the service cannot place at all,
such as a malformed \code{accountId}. A well-formed UUID the service simply has no link for is
the former: the player exists and has a route to fixing it.

Such a response \rec{} carry a human-readable linking instruction, which the client relays
verbatim into chat. It goes in one of two places depending on the response shape, and a service
returning unlinked players through both \req{} populate both:

\begin{itemize}
  \item \textbf{Error envelopes} (\code{/getBalance}, \code{/processTransaction},
        \code{/voidTransaction}) --- in \code{error.details.linkInstructions}.
  \item \textbf{Per-entry lists} (\code{/resolveAccounts} \code{accounts[]},
        \code{/getBalances} \code{unresolved[]}) --- as a \code{linkInstructions} field on the
        entry itself. These are 200 responses where one bad player must not fail the batch, so
        there is no error object to hang it from.
\end{itemize}
