...
- Pocket — three filtered slots (phone / debit card / bills) backed by a server-authoritative capability that survives death/respawn/dimension change. Opens via a keybind (default `P`) into a small GUI that respects shift-click and number-key swap filters. Contents render as a HUD overlay (driven by OneConfig — `cc.polyfrost`) above the hotbar by default; position, scale, background, and border are drag-to-reposition customizable through OneConfig's standard editor (R-SHIFT, or the SUM keybind, or the "Edit HUD…" button on the pocket GUI). The SUM settings page is gated to singleplayer or operator-level players on multiplayer servers.
...
| [OneConfig](https://github.com/Polyfrost/OneConfig) by Polyfrost | LGPL-3.0 + Additional Terms | Hard runtime dependency (`cc.polyfrost:oneconfig-1.12.2-forge:0.2.2-alpha+`). SUM uses OneConfig for client-side config storage and HUD layout management — the pocket HUD extends `BasicHud`, and the SUM mod page registers via `cc.polyfrost.oneconfig.config.Config`. Pinned to the same 0.2.x alpha line that EvergreenHUD 2.x for 1.12.2 uses to avoid version drift in the standard UIA modpack. |
| [EvergreenHUD](https://github.com/Polyfrost/EvergreenHUD) by Polyfrost | GPL-3.0 | Reference for the HUD-element catalog. SUM's information HUDs (Coords, Direction, Biome, Day, Clock, Real-Life Date, Server IP, Resource Pack, Game Mode, Block Above, Height Limit), player-state HUDs (Pitch, Yaw, Speed, Saturation, Armor Durability), performance HUDs (FPS, Memory, Ping, TPS), counter HUDs (CPS, Click Counter, Blocks Placed, Session Playtime), and five user-configurable Custom Text slots follow EvergreenHUD's pattern of one OneConfig HUD module per piece of information. The TPS calculation reuses EvergreenHUD's approach (measure real-time gap between consecutive vanilla `SPacketTimeUpdate` packets). No code reused (upstream is Kotlin, our target is Java + RetroFuturaGradle); the design is reused. Hypixel/PvP-specific elements (BedwarsResource, Combo, Map/MapType, Reach) and complex render elements (CustomImages, Inventory, PlayerPreview, HeldItemLore) are intentionally not ported. |
