...
- Pocket — three filtered slots (phone / debit card / bills) backed by a server-authoritative capability that survives death/respawn/dimension change. Opens via a keybind (default `P`) into a small GUI that respects shift-click and number-key swap filters. Contents render as a HUD overlay above the hotbar by default; the overlay's position and scale are drag-to-reposition customizable via a separate keybind (default `L`), with the saved layout persisted per-client to `config/sum/pocket-hud.json`.
...
| [EvergreenHUD](https://github.com/Polyfrost/EvergreenHUD) by Polyfrost | GPL-3.0 | Inspired the pocket HUD layout editor (`GuiPocketHudEditor`) — drag-to-reposition with on-screen preview, scale stepper, toggle, reset, save-on-close. Upstream is Kotlin and targets 1.8.9 via OneConfig, so no code is reused; SUM's native Java implementation is clean-room from the upstream UX. Pocket HUD layout is persisted client-side at `config/sum/pocket-hud.json` (separate from EvergreenHUD's OneConfig store). |
